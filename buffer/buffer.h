...
public:
    ...
    //缓冲区大小
    inline size_t m_size()const{return m_end-m_elements;}
    //缓存区中可以读取的字节数
    inline size_t writeableBytes() const{return m_size()-m_write_idx;}
    //缓存区中可以写入的字节数
    inline size_t readableBytes() const{return m_write_idx-m_read_idx;}
    //缓存区中已经读取的字节数
    inline size_t haveReadBytes() const{return m_read_idx;}
    //缓冲区中已经写入的字节数
    inline size_t haveWriteBytes() const{return m_write_idx;}
    // 当前的读取指针的位置
    const char *curReadPtr() const {return m_elements + m_read_idx;}
    // 当前的写入指针的位置
    const char *curWritePtr() const {return m_elements + m_write_idx;}
...
private:
    ...
    //不管是从文件读进来的，还是自己写进去要发送给文件的，append会使得m_write_idx增加
